...
//
// 192.168.25.2   25.1+35.2     35.1+45.2     45.1+55.2      55.1+11.2       192.168.11.1
//      STA        AP+STA        AP+STA        AP+STA         AP+STA             AP
//       *           *             *             *              *                *
//   y^  |           |             |             |              |                | 
//    |  U----------R0------------R1------------R2-------------R3----------------S
...
//-----------------------Nodes Define-------------------------------------
  ...
  Ptr<Node> nR3 = CreateObject<Node> ();  // Node4
	Ptr<Node> nS = CreateObject<Node> ();		// Node5

	NodeContainer nodes = NodeContainer (nU, nR0, nR1, nR2, nR3); //max par is 5, use Add to input more
  nodes.Add (nS);
	// Sta-AP links
	NodeContainer nSnR3 = NodeContainer (nS, nR3);
  NodeContainer nR3nR2 = NodeContainer (nR3, nR2);
...
//--------------------MAC and Device Define-------------------------------
	...
	NetDeviceContainer dSdR3;
	dSdR3 = wifi.Install (phy, apmac, nS);
	dSdR3.Add (wifi.Install (phy, stamac, nR3));

  Ssid ssid_55 = Ssid ("openwrt55");
  apmac.SetType ("ns3::ApWifiMac",
               "Ssid", SsidValue (ssid_55));
  stamac.SetType ("ns3::StaWifiMac",
               "Ssid", SsidValue (ssid_55),
               "ActiveProbing", BooleanValue (false));
  NetDeviceContainer dR3dR2;
  dR3dR2 = wifi.Install (phy, apmac, nR3);
  dR3dR2.Add (wifi.Install (phy, stamac, nR2));
...
//-----------------Mobility model(Must for wifi)--------------------------
  ...
  positionAlloc->Add (Vector (32.0, 0.0, 0.0));
...
//-------------------Interface and IP Define------------------------------
	...
	Ipv4InterfaceContainer iSiR3 = address.Assign (dSdR3);

  address.SetBase ("192.168.55.0", "255.255.255.0");
  Ipv4InterfaceContainer iR3iR2 = address.Assign (dR3dR2);
  ...
  Ptr<Ipv4> ipv4R3 = nS->GetObject<Ipv4> ();
...
//----------------------Route Table Define--------------------------------
  ...
  Ptr<Ipv4StaticRouting> staticRoutingR3 = ipv4RoutingHelper.GetStaticRouting (ipv4R3);
  ...
  staticRoutingS->AddNetworkRouteTo (Ipv4Address ("192.168.55.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.11.2"), 1);

  staticRoutingR3->AddNetworkRouteTo (Ipv4Address ("192.168.25.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.55.2"), 2);
  staticRoutingR3->AddNetworkRouteTo (Ipv4Address ("192.168.35.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.55.2"), 2);
  staticRoutingR3->AddNetworkRouteTo (Ipv4Address ("192.168.45.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.55.2"), 2);

  staticRoutingR2->AddNetworkRouteTo (Ipv4Address ("192.168.11.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.55.1"), 1);
  ...
  staticRoutingR1->AddNetworkRouteTo (Ipv4Address ("192.168.55.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.45.1"), 1);
  staticRoutingR1->AddNetworkRouteTo (Ipv4Address ("192.168.11.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.45.1"), 1);
  
  staticRoutingR0->AddNetworkRouteTo (Ipv4Address ("192.168.55.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.35.1"), 1);
  ...
  staticRoutingU->AddNetworkRouteTo (Ipv4Address ("192.168.55.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.25.1"), 1);
...
//-----------------------APP Define---------------------------------------
	...
	UdpClientHelper Client (iSiR3.GetAddress (0), 9);
	Client.SetAttribute ("MaxPackets", UintegerValue (4294967295u));
	Client.SetAttribute ("Interval", TimeValue (Time (time_interval)));	//0.0001~112Mbps
...
/*-----------------------------------------------------*/
	...
	if (tracing == true)
	{
		...
		phy.EnablePcapAll ("WSOL_6n");	//<name>-<node>-<device>.pcap, <node>是在node create时确定的。
  ...
  monitor->SerializeToXmlFile("WSOL_6n.xml", true, true);
