...
//
//  192.168.35.2    35.1+11.2   192.168.11.1
//       STA          AP+STA         AP
//    y   *             *            *
//    |   |             |            | 
//    |   U------------R1------------S
  ...
  Gnuplot2dDataset dataset)
{
  ...
  */
  ...
  // `192.168.0.7` 是client(Node #10)的IP
    if ((t.sourceAddress=="192.168.35.2" && t.destinationAddress == "192.168.11.1"))
    {
        // UDP_PROT_NUMBER = 17
...
//-----------------------Nodes Define-------------------------------------
	...
	Ptr<Node> nR1 = CreateObject<Node> ();	// Node1
	Ptr<Node> nS = CreateObject<Node> ();		// Node2

	NodeContainer nodes = NodeContainer(nU, nR1, nS);
	// Sta-AP links
	NodeContainer nSnR1 = NodeContainer (nS, nR1);
	NodeContainer nR1nU = NodeContainer (nR1, nU);
	// Sta and AP nodes
...
//--------------------MAC and Device Define-------------------------------
	...
	NetDeviceContainer dSdR1;
	dSdR1 = wifi.Install (phy, apmac, nS);
	...
	stamac.SetType ("ns3::StaWifiMac",
							 "Ssid", SsidValue (ssid_35),
							 "ActiveProbing", BooleanValue (false));
	NetDeviceContainer dR1dU;
	dR1dU = wifi.Install (phy, apmac, nR1);
	dR1dU.Add (wifi.Install (phy, stamac, nU));
...
//-------------------Interface and IP Define------------------------------
	...
	Ipv4InterfaceContainer iSiR1 = address.Assign (dSdR1);

	address.SetBase ("192.168.35.0", "255.255.255.0");
	Ipv4InterfaceContainer iR1iU = address.Assign (dR1dU);
...
//----------------------Route Table Define--------------------------------
  ...
  staticRoutingU->AddNetworkRouteTo (Ipv4Address ("192.168.11.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.35.1"), 1);
...
//-----------------------APP Define---------------------------------------
	...
	UdpClientHelper Client (iSiR1.GetAddress (0), 9);
...
/*-----------------------------------------------------*/
	...
	if (tracing == false)
	{
		...
		phy.EnablePcapAll ("WSOL_3n");	//<name>-<node>-<device>.pcap, <node>是在node create时确定的。
  ...
  monitor->SerializeToXmlFile("WSOL_3n.xml", true, true);
